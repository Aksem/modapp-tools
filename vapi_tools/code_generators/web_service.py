...
from vapi_tools.visitor import Visitor
from vapi_tools.protobuf_parser.ast import ProtoModule, Comment, Service, ServiceEndpoint, Package, Message, MessageProperty
from .js_cst import CodeElementList, CodeElement, Import
...
class WebServiceGenerator(Visitor):
    ...
    def visit_MessageProperty(self, msg_property: MessageProperty) -> bool:
        if len(self.current_messages) is 0:
    ...
    def visit_Service(self, service: Service) -> bool:
        ...
        self.code_el_list.append(CodeElement(f'export const {service_name_formatted} = {{'))
    ...
    def visit_ServiceEndpoint(self, endpoint: ServiceEndpoint) -> bool:
        ...
        if endpoint.argument_type in self.local_messages.keys():
            ...
            if self.package_name is not None:
                argument_type_full_name = f"{self.package_name}.{argument_type_full_name}"
                imports.add(Import(name=self.package_name.split('.')[0], default=False, path='./protos.js'))
            else:
                # no package, import message by name
                imports.add(Import(name=argument_type_full_name, default=False, path='./protos.js'))
            ...
            imports.add(Import(name=argument_type_full_name.split('.')[0], default=False, path='./protos.js'))
        ...
        if endpoint.return_type in self.local_messages.keys():
            ...
            if self.package_name is not None:
                ...
                # no package, import message by name
                imports.add(Import(name=return_type_full_name, default=False, path='./protos.js'))
        else:
            # imported message
            return_type_full_name = endpoint.return_type
            # google.rpc.Empty -> import 'google' const
            imports.add(Import(name=return_type_full_name.split('.')[0], default=False, path='./protos.js'))
            # TODO: check whether message exists in source module?
